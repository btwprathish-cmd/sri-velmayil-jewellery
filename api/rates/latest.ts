...
async function fetchFromGoldApi(): Promise<LiveRateRecord | null> {
  try {
    ...
    const record = buildRecord(gold22kPerGram, silverPerGram, "gold-api.com + frankfurter.app");
...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const history = readHistory();
    if (history.length > 0) {
      return res.json({ ...history[0], source: `${history[0].source} (cached)` });
    }

    return res.json(buildRecord(13860, 270, "fallback-default"));
  } catch {
