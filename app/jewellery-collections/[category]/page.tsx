...
export async function generateMetadata({ params }: CategoryPageProps) {
  ...
  return getSeoMetadata({
    title: `${category.name} Collection | Sri Velmayil Jewellery Tirupur`,
    description: `Browse our exquisite, 100% BIS 916 hallmarked ${category.name.toLowerCase()} at Sri Velmayil Jewellery Tirupur. See live price estimates based on today's gold rate.`,
    path: `/jewellery-collections/${slug}`
  });
}
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  // Generate Product Schemas for Google Rich Search Results
  const productSchemas = category.items.map((item) => {
    ...
    const priceWithGst = Math.round(taxable * 1.03);

    return {
      "@context": "https://schema.org",
      "@type": "Product",
      "@id": `https://srivelmayiljewellery.com/jewellery-collections/${slug}#${item.id}`,
      "name": item.name,
      "image": `https://srivelmayiljewellery.com/images/${item.id}-tirupur.webp`,
      ...
      "offers": {
        ...
        "url": `https://srivelmayiljewellery.com/jewellery-collections/${slug}`
      }
    };
  });
      ...
      <div className="py-16 sm:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center md:text-left border-b border-[#D4AF37]/20 pb-8 mb-12">
          ...
          <p className="mt-3 text-sm sm:text-base text-[#F3E5AB]/75 font-sans leading-relaxed max-w-3xl">
            {category.description} All ornaments are crafted in 22K gold, stamped with the government approved BIS logo, and carry a unique HUID.
          </p>
        </div>
              ...
              >
                ...
                <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-[#F3E5AB]/65 font-sans leading-relaxed">
                      {item.description}
                    </p>
                  </div>
          ...
          <div className="flex items-center space-x-4 mb-6 md:mb-0">
            ...
            <div>
              ...
              <p className="text-xs text-[#F3E5AB]/65 font-sans mt-0.5 max-w-xl">
                Every ornament undergoes rigorous purity testing at government-certified hallmarking centers and carries the BIS triangular logo alongside a unique HUID stamp.
              </p>
            </div>
          </div>
          <Link
