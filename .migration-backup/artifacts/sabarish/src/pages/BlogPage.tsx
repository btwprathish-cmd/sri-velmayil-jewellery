    ...
    <div className="py-16 sm:py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-16">
        <div className="inline-flex items-center space-x-2 bg-[#D4AF37]/10 px-3 py-1 rounded-full border border-[#D4AF37]/20 mb-4">
          ...
          <span className="text-[10px] font-bold text-[#D4AF37] uppercase tracking-widest">Jewellery Guides & Tips</span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {(blogPosts as Array<{ slug: string; title: string; excerpt: string; date: string; author: string }>).map((post) => (
          <article key={post.slug} className="bg-[#1a0b2e]/40 border border-[#D4AF37]/15 rounded-2xl overflow-hidden hover:border-[#D4AF37]/45 transition-all duration-300 flex flex-col justify-between shadow-xl">
            ...
            <div className="p-6 flex-grow flex flex-col justify-between space-y-4">
              <div className="space-y-3">
                <div className="flex items-center space-x-4 text-[10px] text-[#F3E5AB]/50 font-sans">
                  <span className="flex items-center"><Calendar className="h-3.5 w-3.5 mr-1 text-[#D4AF37]/80" />{formatIndianDate(post.date)}</span>
                  <span className="flex items-center"><User className="h-3.5 w-3.5 mr-1 text-[#D4AF37]/80" />{post.author}</span>
                </div>
                <h2 className="font-serif text-xl font-bold text-white hover:text-[#D4AF37] transition-colors line-clamp-2">
                  <Link href={`/blog/${post.slug}`}>{post.title}</Link>
                </h2>
                <p className="text-xs text-[#F3E5AB]/65 font-sans leading-relaxed line-clamp-3">{post.excerpt}</p>
              </div>
              <div className="pt-4 border-t border-[#D4AF37]/10">
                <Link href={`/blog/${post.slug}`} className="inline-flex items-center text-xs font-bold text-[#D4AF37] hover:text-[#F3E5AB] group/link">
