...
export default function RateCalculator({ today22kRate }: RateCalculatorProps) {
  ...
  const [makingCharges, setMakingCharges] = useState<number>(10); // 10% default
  const [customRate, setCustomRate] = useState<number>(today22kRate);

  // Derive other purity rates based on 22K rate
  const getRatePerGram = () => {
    if (purity === "24K") {
      // 24K is approximately 22K rate / 0.916
      return Math.round(customRate / 0.916);
    } else if (purity === "18K") {
      // 18K is approximately 22K rate * (18/22)
      return Math.round(customRate * (18 / 22));
    }
    return customRate; // 22K
  };
  ...
  const gstValue = taxableValue * 0.03; // 3% GST
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Live rate per gram"
            />
          </div>
              ...
              {["24K", "22K", "18K"].map((carat) => (
                <button
                  ...
                  className={`py-2 rounded-lg text-sm font-semibold transition-all duration-200 ${
                    purity === carat
                      ? "bg-[#D4AF37] text-[#1a0b2e] shadow-md shadow-[#D4AF37]/15"
          ...
          <div>
            ...
            <div className="flex justify-between text-[10px] text-[#F3E5AB]/40 mt-1">
              ...
              <span>10% (Avg ornaments)</span>
              <span>25% (Heavy designs)</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#1a0b2e]/60 border border-[#D4AF37]/15 rounded-xl p-6 flex flex-col justify-between">
          <div className="space-y-4">
            <h4 className="text-xs font-bold text-[#D4AF37] uppercase tracking-wider border-b border-[#D4AF37]/10 pb-2">
              Price Breakdown
            </h4>
          ...
          <div className="border-t border-[#D4AF37]/25 pt-4 mt-6">
            ...
            <p className="text-[10px] text-[#F3E5AB]/40 leading-relaxed italic flex items-start">
              ...
              Disclaimer: This calculation is an estimate based on daily market rates. Exact rates and making charges may vary by product design at the store.
            </p>
          </div>
        </div>
      </div>
